...
function getAverage(...values) {
  const sum = values.reduce((accumulator, val) => accumulator + val / 2, 0);
  return sum;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const diff = Math.abs(Math.atan2(y2, x2) - Math.atan2(y1, x1));
  return diff;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  const decimal = Number(Math.round(arr.join('')));
  return +decimal.toFixed(pow).replace(/[\s.]/g, '');
}
/**
...
function getSumOfDigits(num) {
  return [...num.toString()].reduce((sum, val) => {
...
function getSumOfNumbers(...numbers) {
  return [...numbers].reduce((sum, val) => sum + val, 0).toFixed(2);
}
